<template>
  <div>
    ...
    <n-list class="main-content-wrap profile-wrap" bordered>
      ...
      <n-spin :show="userLoading">
          ...
          <div class="base-info">
            ...
            <div class="userinfo">
              ...
              <span class="info-item"
                >{{ formatDate(user.created_on) }}&nbsp;加入</span
              >
            </div>
            <div class="userinfo">
              <span class="info-item">
        ...
        <whisper-add-friend
          ...
          :user="user"
          @success="addFriendWhisperSuccess"
        />
      </n-spin>
      <n-tabs
    ...
    <n-space v-if="totalPage > 0" justify="center">
      <InfiniteLoading
        class="load-more"
        :slots="{ complete: '没有更多Aimo了', error: '加载出错' }"
        @infinite="nextPage()"
      >
        <template #spinner>
...
const handleFollowUser = () => {
  dialog.success({
    title: "提示",
    content: "确定" + (user.is_following ? "取消关注" : "关注") + "该用户吗？",
    positiveText: "确定",
    negativeText: "取消",
    onPositiveClick: () => {
      userLoading.value = true;
