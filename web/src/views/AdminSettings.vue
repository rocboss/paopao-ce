...
const inactiveItemCount = computed(() => {
  return inactiveDomains.value.reduce(
...
const isEditable = (schema: SchemaItem) => {
  return (
    schema.active && !schema.readonly && schema.apply_mode !== 'bootstrap_only'
  );
};
...
const extractDraftValue = (schema: SchemaItem): SettingPrimitive => {
  ...
  if (current?.effective_value !== undefined) {
    return normalizeDraftValue(
      schema,
      current.effective_value as SettingPrimitive,
    );
  }
  if (schema.bootstrap_value !== undefined) {
    return normalizeDraftValue(
      schema,
      schema.bootstrap_value as SettingPrimitive,
    );
  }
  return fallbackValueForType(schema);
};
...
const collectChangedItems = ():
  | Api.Admin.NetParams.SettingValueInput[]
  | null => {
